...
Page({
    ...
    async bindtapChangeOrderType(e) {
        let currentTabList = this.data.tabList;
        let currentTab = currentTabList.find(v => {
            return v.checked
        })
    ...
    //投递简历
    catchtapSendResume: async function (e) {
        let that = this;
        // 切换身份
        let switchRole = await iUserRoleService.trySwitchToCandidate();
        if (!switchRole) {
        ...
        try {
            Loading.begin();
            let job = await recruitJobService.loadEntityById(currentItem.jobUuid);
            that.sendjianlixx(currentItem.recruiterOpenid, job.data.jobName, candidateOpenid)
