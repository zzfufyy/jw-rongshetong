...
Page({
	...
	binderrorCompanyPortrait(e) {
		console.error(e);
		this.setData({
			tximg: CONSTANT.STATIC_IMG_URL.portrait_company,
		})
	},
	//工作经历  添加
	gzjltj() {
	...
	async loadContent() {
		...
		loadCompanyPromise.then(r => {
			console.log(r);
			that.setData({
				tximg: img_util.handleCompanyPortraitPath(r.data.portraitPath),
		...
		if (photoPath_list.length > 0) {
			temp_zwpj = photoPath_list.map(val => {
				return { imgqy: app.globalData.web_path + val };
			})
		}
		this.setData({
			zwpj: temp_zwpj,
