...
const createPageMethods = () => ({
    ...
    _loadJobList: async function () {
        // 加载页面 排序参数
        let orderType = this.data.orderTypeList.find(v => {
            return v.checked
        }).type;
        ...
        let newList = dataList.map(r => ({
            ...
            flagApply: Boolean(r.flagApply),
            flagApplyText: Boolean(r.flagApply) ? '已投递' : '投递简历',
            jobRequireMajor: r.jobRequireMajor,
            jobBeginTime: r.jobBeginTime,
            jobEndTime: r.jobEndTime,
        }));
        // 拼接数据
        this.setData({
