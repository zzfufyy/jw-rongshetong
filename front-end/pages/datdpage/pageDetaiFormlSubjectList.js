...
const createPageMethods = () => ({
    // 分页状态
    stateDetailFormSubjectList: {
        pageConfigDetailFormSubjectList: new Paging.PageConfig(4), 
    },
    // 清空内容
    _clearDetailFormSubjectList: function () {
    ...
    // 加载分页内容
    _loadDetailFormSubjectList: async function () {

        // 初始化分页配置
        let pageConfig = this.stateDetailFormSubjectList.pageConfigDetailFormSubjectList;
        ...
        let current = this.data.detailFormSubjectList;
        console.log(current);
        ...
        // 拼接数据
        this.setData({
            detailFormSubjectList: current.concat(newList),
        });
    },
});
