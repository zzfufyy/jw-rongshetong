...
Page({
  ...
  msxx: async function (e) {
                ...
                wx.setEnable1v1Chat({
                    ...
                    success: (e) => {
                        wx.join1v1Chat({
                            ...
                            listener: {
                                nickname: that.data.jobname,
                                openid: that.data.recruitOpenid,
                            },
                        });
                    },
                    fail: (e) => {
	...
	// 已查看 点击事件跳转
	async bindtapJobgtlist(e) {
		let recruitJobUuid = this.data.jobgtlist[e.currentTarget.dataset.index].jobUuid;
		wx.navigateTo({
			url: "/pages/zwxq/zwxq?recruitJobUuid=" + recruitJobUuid,
		})
	},
	// 不合适 点击事件跳转
	async bindtapJobbhslist(e) {
		let recruitJobUuid = this.data.jobbhslist[e.currentTarget.dataset.index].jobUuid;
		wx.navigateTo({
			url: "/pages/zwxq/zwxq?recruitJobUuid=" + recruitJobUuid,
		})
	},
	...
	onLoad: async function (options) {
		...
		await loadRecordListPormise.then(r => {
          ...
          recruitOpenid:v.recruiterOpenid
				}
