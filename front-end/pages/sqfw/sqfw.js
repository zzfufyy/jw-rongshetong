...
// 附加模块
const newsListModule = require('./newsListModule');
...
Page({
    ...
    bindtapGoLink: async function(e) {
        ...
        switch (id) {
            ...
            case 'tagPersonalCenterCommunity':
                ...
                // 如果是招聘者  修改路由到
                if (globalService.getGlobalUserIdentity() == CONSTANT.UserRole.Recruiter) {
                    // 判断是否有公司
                    ...
                    if (string_util.isEmpty(companyUuid)) {
                        console.info(`User Openid: ${this.data.openid} hasn't register Company`);
                        let tryBindCompanyRes = await prompt_util.showModalPrompt("是否尝试绑定平台已有公司")
    ...
    onLoad: async function (options) {
        ...
        console.log(`传入社区id为${options.communityUuid}`);
        ...
        wx.request({
            ...
            success: function (res) {
                ...
                if (res.data.communityInformationList.length > 0) {
                    let communityUuid = res.data.communityInformationList[0].id;
                    that.setData({
                        communityUuid: communityUuid,
                    });
                }
    ...
    ...newsListModule.createPageMethods(),
})